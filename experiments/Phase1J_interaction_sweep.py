...
OUT=Path('/mnt/data/Correlon_Phase1G_to_Phase2'); OUT.mkdir(exist_ok=True)
...
def fit_models(eta,H):
    ...
    for name,fun,p0,bounds,k in [('delta_plus_bg',atom,[.08,.05],([0,0],[np.inf,np.inf]),2),('fractional_cusp',cusp,[.15,.5,.01],([0,0,0],[np.inf,1,np.inf]),3),('smooth_continuum',smooth,[.2,-.05],([-np.inf,-np.inf],[np.inf,np.inf]),2)]:
        p,_=curve_fit(fun,eta,H,p0=p0,bounds=bounds,maxfev=100000); pred=fun(eta,*p); rss=np.sum((H-pred)**2); aic=n*np.log(max(rss/n,1e-300))+2*k; aicc=aic+2*k*(k+1)/(n-k-1); out.append((name,float(aicc),p))
    return out
...
def run(U,L=10,q=np.pi):
    ...
    ev=lines.energy.to_numpy(float); f=lines.weight.to_numpy(float); f/=f.sum(); grid=np.linspace(max(0,ev.min()-1),min(ev.max()+1,50),5001); heights=np.array([broaden(ev,f,e,grid).max() for e in ETAS])
...
df=pd.DataFrame(rows); df.to_csv(OUT/'Phase1J_interaction_sweep.csv',index=False)
